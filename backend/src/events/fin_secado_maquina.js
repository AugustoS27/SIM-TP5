...
export default function finSecadoMaquina(vectorActual, vectorAnterior, reloj, secadora, h) {
  let minIndex = -1;
  ...
  for (let i = 35; i < vectorAnterior.length; i++){
    if (vectorAnterior[i].estado === "siendo secado con secadora"){
      ...
      tiempoSecadoMaquina = reloj - vectorAnterior[i].tiempoComienzoSecadoMaquina; // Acumula el tiempo de secado
    }

    if (vectorAnterior[i].estado === "siendo secado solo"){
      if (minIndex === -1 || vectorAnterior[i].tiempoComienzoSecadoSolo < vectorAnterior[minIndex].tiempoComienzoSecadoSolo) {
        minIndex = i;
      }
    }
  }
  ...
  if (minIndex === -1){
    ...
    let k = 0;
    switch (vectorAnterior[minIndex].tipoVehiculo) {
      case "pequeño":
        k = 0.75;
        break;
      case "mediano":
    ...
    const tiempoEnSecadoSolo = reloj - vectorAnterior[minIndex].tiempoComienzoSecadoSolo;
